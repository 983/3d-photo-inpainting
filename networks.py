...
class Inpaint_Depth_Net(nn.Module):
    ...
    def add_border(self, input, mask_flag, PCONV=True):
        with torch.no_grad():
            ...
            enlarge_input = torch.zeros((input.shape[0], input.shape[1], h + residual_h, w + residual_w)).to(device)
            if mask_flag:
                if PCONV is False:
                    enlarge_input += 1.0
    ...
    def forward_3P(self, mask, context, depth, edge, unit_length=128, cuda=None):
        with torch.no_grad():
            ...
            anchor_h = residual_h//2
            anchor_w = residual_w//2
            enlarge_input = torch.zeros((n, c, h + residual_h, w + residual_w)).to(device)
            enlarge_input[..., anchor_h:anchor_h+h, anchor_w:anchor_w+w] = input
            if cuda is not None:
                if isinstance(cuda, int):
                    enlarge_input = enlarge_input.to(device)
                else:
                    enlarge_input = enlarge_input.to(device)
